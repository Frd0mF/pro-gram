import {useState, useContext, useEffect} from 'react'
import {Link, useNavigate} from 'react-router-dom'
import FirebaseContext from '../context/firebase'

export default function Login() {
    //get navigation history
    const history = useNavigate()
    //get firebase context
    const {firebase} = useContext(FirebaseContext)

    const [emailAdress, setEmailAdress] = useState('')
    const [password, setPassword] = useState('')
    const [error, setError] = useState('')
    const isInvalid = emailAdress === '' || password === ''
    const loginHandle = () => {}

    useEffect (()=>{
        document.title = 'Login - ProGram'
    },[])

    function test () {
        console.log('clicked')
    }
    return(
       <div className="flex text-white font-sans font-light flex-col h-screen bg-primary sm:bg-secondary items-center">
           <img className="w-16 mt-6" src="./images/favicon.png" />
           <h1 className="mt-4 text-2xl">Sign in to ProGram</h1>
           {error && <p className="mb-4 text-sm text-red-600"> {error} </p>}
           <div className="flex flex-col mt-6 w-80 border border-normal rounded-lg">
               <form className="pt-4 px-5 text-sm sm:bg-primary" onSubmit={loginHandle} method="POST">
                <label className="flex flex-col"> Email address
                <input 
                aria-label="Enter your email adress"
                type="text" 
                value = {emailAdress}
                className="text-sm bg-transparent w-full mr-3 mt-2 py-3.5 px-4 h-2 border border-normal rounded-lg"
                onChange ={({target})=>setEmailAdress(target.value)}
                />
                </label>
                <label className="flex flex-col mt-3"> Password
                <input 
                aria-label="Enter your password"
                type="password"
                value = {password}
                className="text-sm bg-transparent w-full mr-3 mt-2 py-3.5 px-4 h-2 border border-normal rounded-lg"
                onChange ={({target})=>setPassword(target.value)}
                />
                </label>
                <input 
                type="button" 
                value="Sign in"
                className={`bg-button my-6 py-1.5 rounded-lg w-full ${!isInvalid && 'hover:bg-purple-900'} ${isInvalid && 'opacity-50'}`}
                onClick={test}
                />
               </form>
           </div>
           <div className="flex flex-row">
            <Link to="/signup" className="cursor-pointer hover:underline mt-8 text-sm text-link">Create an account </Link>
           </div>
       </div>
    )
}